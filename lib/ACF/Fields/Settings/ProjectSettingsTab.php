 ...
 */
class ProjectSettingsTab extends Tab {
    ...
    protected $strings = [
        'tab'           => 'Projektit',
        'projects_page' => [
            'title'        => 'Projektien listaussivu',
            'instructions' => 'Esitetään projektien murupolussa.',
            // phpcs:ignore
        ],
    ];
    ...
    public function sub_fields( $key ) {
        ...
        try {
            $program_page = ( new Field\PostObject( $strings['projects_page']['title'] ) )
                ->set_key( 'projects_page' )
                ->set_name( 'projects_page' )
                ->set_post_types( [ PostType\Page::SLUG ] )
                ->set_return_format( 'id' )
                ->set_instructions( $strings['projects_page']['instructions'] );
